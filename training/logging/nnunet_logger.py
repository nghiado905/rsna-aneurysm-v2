    ...
    def log(self, key, value, epoch: int):
        ...
        if key == 'mean_fg_dice':
            if value is None or (isinstance(value, (float, np.floating)) and np.isnan(value)):
                new_ema_pseudo_dice = np.nan
            else:
                prev_valid_ema = next(
                    (v for v in reversed(self.my_fantastic_logging['ema_fg_dice'])
                     if not (isinstance(v, (float, np.floating)) and np.isnan(v))),
                    None
                )
                new_ema_pseudo_dice = prev_valid_ema * 0.9 + 0.1 * value if prev_valid_ema is not None else value
